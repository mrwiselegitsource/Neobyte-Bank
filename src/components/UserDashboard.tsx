        ...
        <div className="text-center md:text-left flex flex-col md:flex-row items-center justify-between border-b border-zinc-900 pb-6 gap-6">
          <div className="space-y-1.5 text-center md:text-left">
            ...
            <p className="text-xs text-zinc-500 max-w-xl font-sans leading-relaxed">
              Securely orchestrate. Reveal CVVs under biometrics, instantly freeze cards, and write customized tag labels for subscription tracking.
            </p>
          </div>
            ...
            {purchasedCards.map((card) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Awaiting dispatch overlay */}
                  {card.status === 'awaiting_dispatch' && (
                    <div className="absolute inset-0 bg-zinc-950/95 rounded-2xl z-25 flex flex-col items-center justify-center p-6 border border-[#adff2f]/20">
                      <ShieldCheck className="w-10 h-10 text-[#adff2f] animate-pulse mb-3" />
                      <h4 className="text-white text-xs font-black uppercase tracking-widest font-mono">
                        Awaiting Dispatch
                      </h4>
                      <p className="text-[11px] text-zinc-400 mt-1 max-w-xs text-center leading-relaxed font-sans">
                        Our administration team is verifying your payment screenshot. Your card parameters will be securely assigned and displayed here dynamically.
                      </p>
                      {card.paymentScreenshot && (
                        <div className="mt-3 text-center space-y-1">
                          <span className="text-[9px] text-zinc-500 block font-mono uppercase">Your Uploaded Screenshot:</span>
                          <div className="relative w-14 h-14 border border-zinc-900 mx-auto rounded-lg overflow-hidden mt-1 bg-black shadow-md shadow-black">
                            <img src={card.paymentScreenshot} alt="Uploaded receipt screenshot" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {/* Card mockup view */}
                  <div 
                    className={`w-full aspect-[1.58/1] rounded-2xl p-5 bg-gradient-to-br ${
                  ...
                  >
                    {card.isUploadedImage && card.imageURL && !isRevealed ? (
                      // Display purely the custom PNG uploader graphic in place of the card
                      <div className="absolute inset-0 bg-transparent z-10" />
                    ) : (
                      <>
                        {/* Background tint overlay when details are active/decrypted to promote text accessibility contrast */}
                        ...
                        <div className="w-9 h-7 rounded-sm bg-gradient-to-r from-yellow-500 to-yellow-600 border border-yellow-400/50 flex-shrink-0 z-10 relative" />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-mono border-b border-zinc-900 pb-3">
                      <div className="text-left space-y-1">
                        <span className="text-zinc-[550] uppercase block text-zinc-500">Purchase Date</span>
                      ...
                      <div className="text-right space-y-1">
                        <span className="text-zinc-[550] uppercase block text-zinc-500">Security Check</span>
                    ...
                    {/* Static limit display */}
                    <div className="flex items-center justify-between text-[11px] font-mono border-b border-zinc-900 pb-3">
                      <span className="text-zinc-500 uppercase flex items-center gap-1.5">
                        <Sliders className="w-3.5 h-3.5 text-zinc-500" />
                        Card Active Limit
                      </span>
                      <span className="text-[#adff2f] font-bold">${card.limit.toLocaleString()}/mo</span>
                    </div>
